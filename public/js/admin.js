...
const refreshSeatTable = ({ fromSearch = false } = {}) => {
  if (!activeProject) {
...
const renderMerchOrders = () => {
  ...
  if (!merchOrders.length) {
    merchOrdersTableBody.innerHTML = '<tr><td colspan="7">暂无记录。</td></tr>';
    return;
  }
...
if (seatTableSearchInput) {
  seatTableSearchInput.addEventListener('input', (event) => {
    ...
    refreshSeatTable({ fromSearch: true });
  });
